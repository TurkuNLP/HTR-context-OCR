...
def _context_summary_without_arrays(hough_context: dict) -> dict:
    """Return serializable preprocessing metadata from one Hough context."""
    summary = hough_context.get("hough_preprocessing_summary", {}) if isinstance(hough_context, dict) else {}
    return dict(summary) if isinstance(summary, dict) else {}


def _mask_array_from_context(hough_context: dict, key: str, shape: tuple[int, int]) -> np.ndarray:
    """Return one boolean preprocessing mask, or an empty mask with matrix shape."""
...
class CombinationBundleLogger:
    ...
    def _document_metadata(self, doc: SweepDocument) -> dict:
        ...
        return {
            ...
            "hough_preprocessing": {
                "ref_to_pred_summary": _context_summary_without_arrays(doc.ref_to_pred_hough_ctx),
                "ref_to_ref_summary": _context_summary_without_arrays(doc.ref_to_ref_hough_ctx),
                "ref_to_pred_hough_input_mask": "ref_to_pred_hough_input_mask.npy",
                "ref_to_pred_region_of_interest_mask": "ref_to_pred_region_of_interest_mask.npy",
                "ref_to_pred_strong_match_mask": "ref_to_pred_strong_match_mask.npy",
                "ref_to_ref_hough_input_mask": "ref_to_ref_hough_input_mask.npy",
                "ref_to_ref_region_of_interest_mask": "ref_to_ref_region_of_interest_mask.npy",
                "ref_to_ref_strong_match_mask": "ref_to_ref_strong_match_mask.npy",
            },
    ...
    def _write_completed_document(
        ...
        ref_to_pred_shape = tuple(int(value) for value in doc.ref_to_pred_matrix.shape)
        ref_to_ref_shape = tuple(int(value) for value in doc.ref_to_ref_matrix.shape)
        np.save(
            document_dir / "ref_to_pred_hough_input_mask.npy",
            _mask_array_from_context(doc.ref_to_pred_hough_ctx, "hough_mask_bool", ref_to_pred_shape),
        )
        np.save(
            document_dir / "ref_to_pred_region_of_interest_mask.npy",
            _mask_array_from_context(doc.ref_to_pred_hough_ctx, "region_of_interest_mask_bool", ref_to_pred_shape),
        )
        np.save(
            document_dir / "ref_to_pred_strong_match_mask.npy",
            _mask_array_from_context(doc.ref_to_pred_hough_ctx, "strong_match_mask_bool", ref_to_pred_shape),
        )
        np.save(
            document_dir / "ref_to_ref_hough_input_mask.npy",
            _mask_array_from_context(doc.ref_to_ref_hough_ctx, "hough_mask_bool", ref_to_ref_shape),
        )
        np.save(
            document_dir / "ref_to_ref_region_of_interest_mask.npy",
            _mask_array_from_context(doc.ref_to_ref_hough_ctx, "region_of_interest_mask_bool", ref_to_ref_shape),
        )
        np.save(
            document_dir / "ref_to_ref_strong_match_mask.npy",
            _mask_array_from_context(doc.ref_to_ref_hough_ctx, "strong_match_mask_bool", ref_to_ref_shape),
        )
