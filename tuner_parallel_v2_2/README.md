...
`tuner_parallel_v2_2` is the current Hough Line Transform parameter tuner for score-matrix based document alignment experiments.
...
```bash
bash /scratch/project_2017385/dorian/Churro_copy/tuner_parallel_v2_2/run_hough_parameter_sweep_20nodes_10docs_each.sh \
  ...
  --output-dir /scratch/project_2000539/dorian/results/tuner_parallel_v2_2_all_docs_dynamic_pool_20shards_th10_35_len5_25_gap0_15 \
  --max-items 1170 \
  --shard-count 20 \
  ...
  --use-matrix-cache \
  --matrix-cache-dir /scratch/project_2000539/dorian/cache/tuner_parallel_v2_2/matrix_npz_cache_ws50_st35 \
  --ref-to-ref-cache-mode auto \
  --ref-to-ref-cache-dir /scratch/project_2000539/dorian/cache/tuner_parallel_v2_2/ref_to_ref_combo_cache_ws50_st35 \
  --cpus-per-task 128 \
  --mem 64G \
...
PYTHONPATH=/scratch/project_2017385/dorian/Churro_copy \
  /appl/soft/ai/wrap/pytorch-2.9/bin/python3 tuner_parallel_v2_2/run_hough_parameter_sweep.py \
    --runfile-json /scratch/project_2017385/dorian/Churro_copy/results/custom_churro_infer_dev_full1170_run1/vllm/dev/outputs.json \
    --output-dir /scratch/project_2017385/dorian/Churro_copy/tuner_parallel_v2_2_debug_run \
    --max-items 5 \
...
```python
from tuner_parallel_v2_2.filtering.line_filtering_v2_1_IoU_fast import filter_lines_for_alignment_by_ownership
```
...
The tuner is intended to be understandable from inside `tuner_parallel_v2_2/`. The v2.12 metric code required at runtime is vendored locally:
...
Historical threshold experiments and long-form analysis notes remain outside the v2_2 package tree.
