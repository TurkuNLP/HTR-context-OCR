...
# Precompute the matrix-dependent Hough input once per document.
def precompute_hough_context(
    ...
    *,
    ...
    """Build the Region of Interest Hough context used by v2_2.

    """
    return build_region_of_interest_hough_context(
        matrix,
        config=config,
...
def detect_and_filter_lines_from_matrix(
    ...
    *,
    ...
    mat = coerce_score_matrix(matrix, source_desc="tuner_parallel_v2_2:line_alignment_pipeline_fast:matrix")

    resolved_ctx = hough_ctx
    if resolved_ctx is None:
        resolved_ctx = precompute_hough_context(mat)
...
def derive_filtered_line_endpoints(
    *,
    ...
    if precomputed_matrix is None:
        ...
        matrix = coerce_score_matrix(precomputed_matrix, source_desc="tuner_parallel_v2_2:precomputed_matrix")

    hough_ctx = precompute_hough_context(matrix)
