...
def _save_y_axis_count_visualisation(
    *,
    refref_y: np.ndarray,
    other_y: np.ndarray,
    y_diff: np.ndarray,
    ...
    """Render Y-axis count coverage and subtraction diagnostics.

    Top subplot: ref->ref and ref->other per-character counts.
    Bottom subplot: y_diff = other_y - refref_y with zero baseline.
    """
    out_path.parent.mkdir(parents=True, exist_ok=True)
...
def _save_x_axis_count_visualisation(
    *,
    other_x: np.ndarray,
    out_path: Path,
    file_name: str,
) -> str:
    """Render X-axis coverage counts used for hallucination metric.

    We visualize the ref->other x-axis per-character count profile and explicitly
    mark zero-count positions (these correspond to hallucination by definition).
    """
    out_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    zero_mask = other_x == 0
    if np.any(zero_mask):
        ax.scatter(xs[zero_mask], np.zeros(int(np.count_nonzero(zero_mask))), s=4, color="#ff7f0e", alpha=0.8, label="x==0")
...
def save_count_line_coverage_visualisations(
    *,
    ...
    x_path = _save_x_axis_count_visualisation(
...
def save_text_metrics_visualisations(
    *,
    ...
    if matrix_after_reordering is not None:
        # Keep this branch untouched in behavior. The caller decides whether
        # it wants to provide a matrix for this optional visual.
