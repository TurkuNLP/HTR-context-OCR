...
SCORES_PKL = PROJECT_ROOT / "responses/finnish_custom_infer_run5/vllm/dev/scores.pkl"
...
TARGET_GRAPHS = [
    "0002_europeana_00674548_full.png",
    "0005_europeana_00674591_full.png",
    "0007_europeana_00675330_full.png",
    "0010_europeana_00675472_full.png",
    "0011_europeana_00675473_full.png",
    "0022_newseye-fin_576474_0003_23676390_full.png",
    "0023_newseye-fin_576485_0001_23676428_full.png", #this one is questionable
]

# Allow skipping missing targets instead of raising an error.
ALLOW_MISSING = os.getenv("ALLOW_MISSING", "0") == "1"
...
def main():
    ...
    if missing:
        msg = f"Missing target items in scores.pkl: {missing}"
        if ALLOW_MISSING:
            print(f"WARNING: {msg}")
        else:
    ...
    for graph_name in TARGET_GRAPHS:
        ...
        item = items_by_safe.get(safe_base)
        if item is None:
            # Skip missing targets if ALLOW_MISSING is enabled.
            if ALLOW_MISSING:
                continue
            raise RuntimeError(f"Missing target item in scores.pkl: {safe_base}")
