...
RESULTS_DIR = PROJECT_ROOT / "results/visualise_dorian_dense_matrices_style_single_no_angle"
...
# Target case (filename in scores.pkl)
TARGET_NAME = "newseye-fre_p1020482.jpeg"
...
def detect_lines_dense_style_no_angle(matrix):
    ...
    # This avoids a few very bright pixels stopping the loop too early.
    ...
    # Reduce line_gap to avoid bridging far-apart fragments.
...
with open(SCORES_PKL, "rb") as f:
    while True:
        try:
            ...
            break
        if Path(item["fname"]).name == TARGET_NAME:
            target_item = item
            break

if target_item is None:
    raise FileNotFoundError(f"Target not found in scores.pkl: {TARGET_NAME}")

fig, (ax_img, ax_hm) = plt.subplots(1, 2, figsize=(12, 5))

img_path = IMG_DIR / target_item["fname"]
if img_path.exists():
    ax_img.imshow(plt.imread(img_path))
    ax_img.set_title(target_item["fname"])
    ax_img.axis("off")
else:
    ax_img.text(0.5, 0.5, f"Missing image:\n{target_item['fname']}", ha="center", va="center")
    ax_img.axis("off")

matrix = safe_matrix(target_item.get("scores"))
...
for p0, p1 in det["merged_lines"]:
    ax_hm.plot((p0[0], p1[0]), (p0[1], p1[1]), color="red", linewidth=3)
...
base_name = safe_name(target_item["fname"])
full_out = FULL_DIR / f"{base_name}_full.png"
graph_out = GRAPH_DIR / f"{base_name}_graph.png"
mask_out = MASK_DIR / f"{base_name}_mask.png"
...
if RENDER_NOTEBOOK_OUTPUT:
    pred_esc = html.escape(target_item["pred"])
    ref_esc = html.escape(target_item["ref"])
    display(
        HTML(
            f"""
