...
from .hough_eval import pick_better_eval
...
def merge_best_value_payloads(target: dict[int, dict | None], source: dict[int, dict | None]) -> None:
    """Merge source best rows into target using the tuner ranking function."""
    for value, candidate_row in source.items():
        normalized_value = int(value)
        target[normalized_value] = pick_better_eval(target[normalized_value], candidate_row)
