...
from .filter_geometry_helpers import set_iou
...
def coverages_merge_candidate(coverage_a: dict, coverage_b: dict, *, min_iou_threshold: float) -> bool:
    """Return the exact true-IoU merge decision for two coverage objects."""
...
def coverage_components_for_production(
    ...
    *,
    ...
    for left_index, right_index in iter_possible_overlap_pairs_for_production(coverages):
        ...
        should_merge = coverages_merge_candidate(
...
def run_production_filtering_pipeline(
    ...
    *,
    ...
    candidate_lines = prepare_candidate_lines(lines, matrix, abs_min_len=abs_min_len, profile=profile)
    if profile is not None:
        add_profile_seconds(
            profile,
    ...
    merged_coverages = [
        merge_overlap_component([candidate_coverages[coverage_index] for coverage_index in component], matrix)
