...
STITCHED_BEST_COMBINATION_PANEL_FILENAME = (
    "stitched_best_combination_no_hough_raw_hough_surviving_lines_panel.png"
)
PER_DOCUMENT_BEST_COMBINATION_METRICS_FILENAME = "best_combination_metrics.txt"
PER_DOCUMENT_BEST_COMBINATION_PANEL_FILENAME = "best_combination_no_hough_raw_hough_surviving_lines_panel.png"
...
TEMPORARY_BEST_COMBINATION_PANEL_FILENAME_PREFIX = (
    "best_combination_no_hough_raw_hough_surviving_lines_panel"
)
...
def split_documents_by_bundle_availability_and_prediction(
    *,
    ...
    # Loadable documents have a bundle and at least one prediction column.
    loadable_documents: list[dict[str, Any]] = []

    # Skipped documents are reported with an explicit skip_reason.
    ...
    for runfile_document in runfile_documents:
        ...
        # Missing bundles are expected while the tuner run is still writing.
...
def draw_score_matrix_heatmap(axis, score_matrix: np.ndarray | None, title: str):
    ...
    # origin=upper keeps matrix row 0 at the top.
    image = axis.imshow(score_matrix, origin="upper", aspect="auto", cmap="viridis")
    axis.set_title(title)
    axis.set_xlabel("Other/prediction window index")
    axis.set_ylabel("Reference window index")
...
def draw_line_label_at_segment_midpoint(
    axis,
    *,
    x0: float,
    y0: float,
    ...
    """Draw one readable label near the midpoint of an already-plotted line."""
    midpoint_x = (float(x0) + float(x1)) / 2.0
    midpoint_y = (float(y0) + float(y1)) / 2.0
    axis.text(
        midpoint_x,
        midpoint_y,
        str(label_text),
...
def draw_endpoint_segments(
    ...
    *,
    ...
    """Draw line endpoint segments and return how many were drawn."""
    ...
    for segment_index, (x0, y0, x1, y1) in enumerate(endpoint_segment_list):
        ...
        if show_line_labels and segment_index < len(label_list):
            draw_line_label_at_segment_midpoint(
                axis,
                x0=x0,
                y0=y0,
                x1=x1,
...
def select_best_combination_rows_per_document(pair_metrics_dataframe: pd.DataFrame) -> pd.DataFrame:
    """Return the highest-tuning-score row for every loaded document."""
    ...
    # Only finite tuning scores can be ranked.
    ...
    # Tie-break by smaller threshold/length/gap for stable and explainable output.
...
def render_best_combination_visual_panel(
    *,
    ...
    # The figure uses a third row for complete metrics text.  That keeps all matrix
    # panels readable while ensuring the stitched PNG contains every reported metric.
    fig = plt.figure(figsize=(20, 18.8), constrained_layout=False)
    ...
    metrics_axis = fig.add_subplot(grid_specification[2, :])
    fig.suptitle(
        f"{language_name} / {document_type} | document {document_index} | {document_name}\n"
        "No-Hough score matrices and best-combination ref_to_pred Hough overlays",
        fontsize=15,
        y=0.995,
    )
    ...
    # Add colorbars only for axes that have actual image data.
    ...
    fig.subplots_adjust(left=0.06, right=0.94, bottom=0.04, top=0.92, hspace=0.34, wspace=0.20)
    fig.savefig(output_png_path, dpi=saved_figure_dpi, bbox_inches="tight")
    plt.close(fig)
    return output_png_path
...
def stitch_best_combination_visual_panels_for_pair(
    *,
    ...
    # Pillow is imported lazily so CSV-only or graph-only runs do not require it at startup.
    from PIL import Image

    stitched_output_path.parent.mkdir(parents=True, exist_ok=True)
    panel_images = []
    try:
        # Convert every panel to RGBA so alpha composition and white padding behave uniformly.
        for temporary_panel_path in temporary_panel_paths:
...
def render_best_combination_visual_panels_for_pair(
    *,
    ...
    """Render temporary best panels and publish one stitched panel for the pair."""
    if best_rows_dataframe.empty:
        return None

    first_best_row = best_rows_dataframe.iloc[0]
    language_name = str(first_best_row["main_language"])
    document_type = str(first_best_row["document_type"])
    ...
    temporary_panel_paths: list[Path] = []
    for row_position, (_, best_row) in enumerate(best_rows_dataframe.iterrows(), start=1):
        print(
            f"[{best_row['main_language']} / {best_row['document_type']}] "
            f"temporary best visual panel {row_position}/{len(best_rows_dataframe)}: "
            f"{int(best_row['document_index'])} {best_row['fname']}"
        )
        temporary_panel_paths.append(
    ...
    stitched_panel_path = stitch_best_combination_visual_panels_for_pair(
        temporary_panel_paths=temporary_panel_paths,
        stitched_output_path=stitched_output_path,
    )

    # Temporary files are removed only after a successful stitch so failures remain debuggable.
    if stitched_panel_path is not None:
        remove_temporary_best_combination_panel_files(temporary_panel_paths, temporary_panel_output_dir)

    return stitched_panel_path
...
def dataframe_from_document_records(document_records: list[dict[str, Any]]) -> pd.DataFrame:
    ...
    normalized_records = [
        {key: json_ready_value(value) for key, value in record.items()}
