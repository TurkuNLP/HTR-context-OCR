...
def _compute_y_axis_percentage_metrics(y_diff: np.ndarray) -> dict:
    ...
    IMPORTANT: Category semantics are intentionally unchanged.
    - missing: y_diff == -1
    - ok: y_diff == 0
    - repetition: y_diff > 0

    Any value outside those categories raises, preserving the strict behavior
...
def build_line_coverage_arrays_from_bundles(
    *,
    ...
    """Build coverage arrays once and reuse them for metrics/visualization.

    This function centralizes the bundle->array conversion so we do not duplicate
    interval accumulation logic in multiple call sites.

    Returns a dictionary with:
    - refref_y: per-character coverage counts on reference axis for ref->ref lines
    - other_y: per-character coverage counts on reference axis for ref->other lines
    - other_x: per-character coverage counts on other axis for ref->other lines
    - y_diff: subtraction other_y - refref_y (strict category validation is separate)
    """
...
def compute_line_coverage_percentage_metrics_from_arrays(
    *,
    ...
    This helper intentionally reuses the strict y-axis category function unchanged.
    """
    y_diff = np.asarray(y_diff, dtype=np.int32)
    other_x = np.asarray(other_x, dtype=np.int32)
...
def compute_line_coverage_percentage_metrics_from_bundles(
    *,
    ...
    arrays = build_line_coverage_arrays_from_bundles(
        refref_bundle=refref_bundle,
        other_bundle=other_bundle,
    )
    return compute_line_coverage_percentage_metrics_from_arrays(
        y_diff=arrays["y_diff"],
        other_x=arrays["other_x"],
        file_name=file_name,
    )
