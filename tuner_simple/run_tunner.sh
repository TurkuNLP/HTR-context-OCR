...
Matrix and preprocessing options:
  ...
  --score-floor-alpha <value>            Alpha in score_floor = mean + alpha * standard_deviation
...
# Resolve the real tuner_simple directory, even when Slurm runs a copied script from /var/spool/slurmd.
SCRIPT_SOURCE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Allow advanced users or Slurm wrappers to point directly at the checked-out tuner_simple directory.
...
elif [[ -f "/scratch/project_2017385/dorian/Churro_copy/tuner_simple/run_tuner_simple.py" ]]; then
  SCRIPT_DIR="/scratch/project_2017385/dorian/Churro_copy/tuner_simple"
else
  echo "ERROR: Could not locate run_tuner_simple.py. Set TUNER_SIMPLE_DIR=/path/to/tuner_simple." >&2
  exit 2
fi
