...
# These imports intentionally point to the local repository modules loaded from
# Churro_copy project root, not from any globally installed `churro` package.
from systems.detect_layout import get_total_azure_cost
...
def _build_per_document_metrics(outputs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Return a stable per-document metrics list preserving dataset order."""
    per_document_metrics: list[dict[str, Any]] = []
    for index, output in enumerate(outputs):
        per_document_metrics.append(
            {
                "document_index": index,
                **output,
            }
        )
    return per_document_metrics
...
def compute_metrics(
    ...
    # Persist explicit per-document metrics artifact (one object per dataset item).
    per_document_metrics = _build_per_document_metrics(outputs)
    with open(f"{output_prefix}/per_document_metrics.json", "w") as f:
        json.dump(per_document_metrics, f, indent=2, ensure_ascii=False)
