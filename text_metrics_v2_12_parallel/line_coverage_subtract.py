...
This module receives prebuilt line bundles, converts them into per-character
coverage arrays once, and computes the percentage metrics from those arrays.
The current implementation uses the non-legacy bundle field names while keeping
all coverage values and percentage semantics identical to the previous code.
"""
...
# Compute missing/ok/repetition ratios from the reference-axis subtraction array.
def _compute_y_axis_ratio_metrics(y_diff: np.ndarray) -> dict:
    """Compute missing/ok/repetition ratios from y-axis subtraction.

    The returned values are normalized ratios in ``[0, 1]``.  The old public
    percentage wrapper below multiplies these values by ``100`` only for callers
    that still need the historical percent schema.
    """
    ...
    # ``-1`` means reference-self covered this character but ref-to-pred did not.
    missing_count = int(np.count_nonzero(y_diff == -1))
    # ``0`` means ref-to-pred coverage matches reference-self coverage exactly.
    ok_count = int(np.count_nonzero(y_diff == 0))
    # Values above zero mean ref-to-pred covered this reference character more often.
...
# Compute hallucination ratio from zero prediction-axis coverage.
...
# Build the reference-self y-axis coverage counts from one v2.12 bundle.
...
# Build the ref-to-other coverage arrays that depend on the prediction side.
...
# Build reusable coverage arrays from a cached reference-self coverage baseline.
...
# Build the reusable coverage arrays from the ref->ref and ref->pred line bundles.
...
# Compute normalized ratio metrics from precomputed coverage arrays.
...
# Compute historical percentage metrics from precomputed coverage arrays.
