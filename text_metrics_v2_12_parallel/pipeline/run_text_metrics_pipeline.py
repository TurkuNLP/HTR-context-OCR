"""Top-level orchestration for the text-metrics report pipeline."""
...
from accelerators.build_optional_exact_result_cython_backends import (
...
from pipeline.report_item_views import build_public_non_debug_report_item
...
from pipeline.runtime_artifact_paths import score_index_cache_root
...
def main() -> None:
    ...
    optional_backend_status = prepare_optional_exact_result_cython_backends()
    print(f"[accelerators] {optional_backend_status['message']}")
    ...
    if debug_enabled:
        from debug.run_timing_telemetry import (
            create_debug_timing_spool_file,
            write_final_debug_timing_report,
        )

    score_index_by_kind: dict[str, dict[str, dict]] = {}
    external_score_index_cache_root = score_index_cache_root()
    for kind, path in scores_pkl_paths_by_kind.items():
        if path is None:
            score_index_by_kind[kind] = {}
            continue
        score_index_by_kind[kind] = build_score_stream_index_cached(Path(path), external_score_index_cache_root)
    ...
    print(f"[run] hough_handoff_mode={args.hough_handoff_mode}")
    ...
    timing_spool_unsorted = create_debug_timing_spool_file(args.output_dir) if debug_enabled else None
    ...
    try:
        ...
        write_payload_with_items_stream(
            ...
            item_transform=None if debug_enabled else build_public_non_debug_report_item,
        )
        ...
        if debug_enabled and timing_spool_unsorted is not None:
            out_timing, timing_spool_sorted = write_final_debug_timing_report(
                output_dir=args.output_dir,
                timing_spool_unsorted=timing_spool_unsorted,
                matched=matched,
                attempted=attempted,
                state=state,
                args=args,
                available_cpus=available_cpus,
                hough_params_meta=hough_params_meta,
                levenshtein_backend=LEVENSHTEIN_BACKEND,
            )
