"""Production true-IoU line filtering for the text-metrics pipeline.

This module contains only the production filtering path used by
``run_text_metrics_report.sh``. The expensive debug-only pairwise IoU analysis
lives in ``debug/line_filtering_v2_12_detailed_iou_analysis.py`` and is kept
out of the normal runtime path.

Key guarantees preserved by this implementation:
- candidate preparation thresholds stay unchanged
- per-column line sampling semantics stay unchanged
- true IoU merge rule stays unchanged
- connected-component membership stays unchanged
- local per-column winner selection stays unchanged
- final ownership arrays and output ordering stay unchanged
"""
...
from accelerators.true_iou_filter_backend import (
...
def _ref_segments_from_path(x_to_y: dict[int, int]) -> set[int]:
    ...
    # Add the sampled row at every x and bridge vertical gaps between neighbours.
...
def _fit_line_from_path(
    ...
    *,
    ...
    if not x_to_y:
        ...
        fallback_geometry["score"] = float(fallback_geometry.get("score", fallback_geometry.get("support", 0.0)))
    ...
    if len(sampled_prediction_columns) == 1 or np.allclose(sampled_prediction_columns, sampled_prediction_columns[0]):
...
def _coverage_from_path(
    *,
    ...
    return {
        ...
        "x_to_y": {int(prediction_column): int(reference_row) for prediction_column, reference_row in x_to_y.items()},
        "x_to_score": {
...
def _build_line_coverage(line: dict, matrix: np.ndarray) -> dict | None:
    ...
    sampled_line_path = sample_line_path_with_optional_accelerator(
        line,
        matrix,
        line_x_bounds_function=_line_x_bounds,
        line_y_at_x_function=line_y_at_x,
    )
    if sampled_line_path is None:
        return None

    x_to_y, x_to_score = sampled_line_path
...
# Compute the exact set IoU used by the v2.12 overlap rule.
def _set_iou(values_a: set[int], values_b: set[int]) -> float:
    """Return exact set IoU, preserving the current empty-union behavior."""
...
def _merge_component(component_coverages: list[dict], matrix: np.ndarray) -> dict:
    ...
    merged_x_to_y, merged_x_to_score, merged_source_raw_line_ids = (
        select_component_local_winners_with_optional_accelerator(
            component_coverages,
            local_path_key_function=_local_path_key,
        )
    )
...
def _iter_possible_overlap_pairs_for_production(coverages: list[dict]):
    ...
    for current_index in sorted_coverage_indices:
        ...
        for active_index in active_prediction_overlap_indices:
            ...
            # Once prediction bounds are disjoint, the pair can never merge under the exact IoU rule.
            if active_prediction_max < current_prediction_min:
                continue

            still_active_indices.append(int(active_index))

            # The production path keeps only pairs that overlap on both axes before exact IoU work.
...
# Production-only overlap graph that avoids building debug payloads.
def _coverage_components_for_production(
    coverages: list[dict],
    *,
    min_iou_threshold: float,
) -> list[list[int]]:
    """Build connected components for the production pipeline only."""
    if not coverages:
        return []
...
# Lean production filtering path used by the main report pipeline.
def _run_production_filtering_pipeline(
    ...
    *,
    ...
    """Run the exact filtering semantics without the debug-heavy payload creation."""
...
def filter_lines_for_alignment_by_ownership(
    ...
    **_ignored,
    ...
    """Run the production true-IoU filter and return final lines plus ownership arrays."""
    if not lines:
        n_prediction_columns = matrix.shape[1] if matrix.ndim == 2 else 0
        return [], _empty_assignment(n_prediction_columns)
