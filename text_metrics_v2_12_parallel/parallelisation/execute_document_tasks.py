...
# Build the minimal pickle-friendly argument payload sent into worker processes.
...
# Apply optional per-document Hough overrides to one worker payload.
def build_effective_worker_args_payload(
    *,
    ...
    effective_payload = dict(base_args_payload)
    if not per_doc_hough_params_by_fname:
        return effective_payload

    file_name = Path(str(item_fname)).name
    override = per_doc_hough_params_by_fname.get(file_name)
    if not override:
        return effective_payload

    for key in ("hough_threshold", "hough_line_length", "hough_line_gap", "hough_seed"):
        if key in override:
            effective_payload[key] = int(override[key])

    return effective_payload


# Extract only the score-index entries needed by one document task.
...
# Build a standard skip envelope for empty-prediction items.
...
# Run one non-empty document task and normalize it into a success/failed envelope.
...
# Convert executor-level failures into the standard failed envelope schema.
...
# Consume completed futures and persist their envelopes immediately.
def consume_done_futures(
    *,
    ...
    for done_future in done_futures:
        future_meta = in_flight.pop(done_future)
        try:
            envelope = done_future.result()
        except Exception as exc:
            envelope = build_failed_envelope_from_executor_exception(
                seq_id=int(future_meta["seq_id"]),
                item_index=int(future_meta["item_index"]),
                item_fname=str(future_meta["item_fname"]),
                exc=exc,
                start_time=future_meta.get("start_time"),
                debug_enabled=bool(debug_enabled),
            )
