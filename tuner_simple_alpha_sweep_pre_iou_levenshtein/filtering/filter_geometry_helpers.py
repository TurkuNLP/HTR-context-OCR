...
try:
    from ..runtime.runtime_paths import ensure_runtime_paths
except ImportError:
    from runtime.runtime_paths import ensure_runtime_paths  # type: ignore

# Direct script execution can place only ``filtering/`` on sys.path.  The local
# runtime helper adds the tuner root and project root before sibling imports.
...
def line_endpoint_deltas_left_to_right(line: dict) -> tuple[float, float]:
    """Return endpoint deltas after orienting one line from smaller x to larger x."""
    ...
    return float(x0 - x1), float(y0 - y1)


def visual_angle_degrees_for_falling_line(line: dict) -> float | None:
    """Return the visual falling-line angle, or None when the line is flat, vertical, or rising."""
    delta_x, delta_y = line_endpoint_deltas_left_to_right(line)
    if delta_x <= 0.0 or delta_y <= 0.0:
        return None
    return float(math.degrees(math.atan2(delta_y, delta_x)))


def line_is_falling_diagonal_in_hough_angle_range(line: dict) -> bool:
    """Return whether a line follows the same falling 30-60 degree range accepted by Hough."""
    visual_angle_degrees = visual_angle_degrees_for_falling_line(line)
    if visual_angle_degrees is None:
        return False
    return bool(
...
__all__ = [
    ...
    "FALLING_DIAGONAL_MIN_VISUAL_ANGLE_DEGREES",
    "fit_line_from_path",
    "line_endpoint_deltas_left_to_right",
    ...
    "set_iou",
    "visual_angle_degrees_for_falling_line",
