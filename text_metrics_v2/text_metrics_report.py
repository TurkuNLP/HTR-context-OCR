...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--debug",
        action="store_true",
        help="Write per-document timing telemetry JSON files.",
    )
    return p.parse_args()
...
def process_item(
    ...
    *,
    ...
    # Ensure ref->ref matrix is always available for metrics paths and auditability.
    t0 = time.perf_counter()
    ...
    t0 = time.perf_counter()
    line_metric = compute_levenshtein_metrics(
        ref_text=ref,
        other_text=pred,
    ...
    line_coverage_metrics = compute_line_coverage_percentage_metrics_from_precomputed_endpoints(
        ...
        refref_line_endpoints=lines_used_ref_to_ref,
...
def main() -> None:
    ...
    for item in run_items:
        ...
        out_report = args.output_dir / f"{case_prefix}.json"

        report = dict(res)
        timing = report.pop("__timing", {})
        out_report.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")

        if bool(args.debug):
            out_timing = args.output_dir / f"{case_prefix}_timing.json"
            timing_payload = {
                "index": int(report["index"]),
                "fname": str(report["fname"]),
        ...
        if bool(args.debug):
            row["timing_path"] = str(args.output_dir / f"{case_prefix}_timing.json")
        rows.append(row)
