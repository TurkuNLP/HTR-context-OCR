...
from tuner_simple_alpha_sweep.config.pipeline_config import PipelineConfig
from tuner_simple_alpha_sweep.document_selection.runfile_loader import RunfileDocument
from tuner_simple_alpha_sweep.matrix_operations.matrix_loader import (
...
from tuner_simple_alpha_sweep.matrix_operations.matrix_shape import (
    # Pass this value into the surrounding multi-line call or collection.
...
from tuner_simple_alpha_sweep.probabilistic_hough.hough_detection import HoughFilteredPayload, run_probabilistic_hough_and_filter
from tuner_simple_alpha_sweep.scoring.line_text_similarity import filter_lines_by_minimum_normalised_levenshtein
from tuner_simple_alpha_sweep.scoring.scoring_pipeline import ScoredDocumentResult, score_document_alignment, zero_alignment_metrics
from tuner_simple_alpha_sweep.scoring.scoring_pipeline import DocumentAlignmentMetrics
...
def empty_hough_payload(*, score_floor_result: ScoreFloorResult) -> HoughFilteredPayload:
    ...
    from tuner_simple_alpha_sweep.probabilistic_hough.hough_input import build_simple_hough_context
...
def harmonic_selection_score(metrics: DocumentAlignmentMetrics) -> float:
    """Return 3 / ((1 / line_nls) + (1 / coverage) + (1 / non_hallucination))."""

    metric_values = (
        metrics.weighted_along_lines_normalised_levenshtein,
        metrics.correct_ref_coverage,
    ...
    return float(3.0 / sum(1.0 / value for value in cleaned_values))
...
def run_alpha_candidate(
    *,
    ...
    line_filter_result = filter_lines_by_minimum_normalised_levenshtein(
        ...
        minimum_line_nls=candidate_config.min_surviving_line_nls,
    )
    ...
    selection_score = harmonic_selection_score(scored.metrics)
...
def process_one_document(
    *,
    ...
    try:
        ...
        selected_result_row: dict[str, Any] | None = None
