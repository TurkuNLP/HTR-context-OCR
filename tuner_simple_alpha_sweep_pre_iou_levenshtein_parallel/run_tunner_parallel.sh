#!/usr/bin/env bash
#SBATCH --job-name=tuner_simple_parallel
...
#SBATCH --output=logs/tuner_simple_parallel_%j.out
#SBATCH --error=logs/tuner_simple_parallel_%j.err
# Run this file with Bash so arrays, functions, and strict error handling behave consistently.
# NOTE: --cpus-per-task controls the alpha worker pool size (floor(cpus/2) by default).
#       Set it to 8 for 4 alpha workers, 16 for 8 workers, etc.
...
usage() {
  ...
  cat <<'USAGE'
Usage: run_tunner_parallel.sh [options]
...
Parallel two-phase options:
  --two-phase                           Enable two-phase scout+refine (default); fast scout ranks alphas, full refine selects winner
  --no-two-phase                        Disable two-phase; run all alphas at full --hough-num-runs (still parallelized)
  --scout-hough-runs <n>                Hough runs per alpha in the cheap scout pass (default 3)
  --refine-top-k <n>                    How many top scout alphas are re-run at full --hough-num-runs (default 5)
  --alpha-parallel-workers <n>          Worker processes for alpha evaluation; 0 = auto = max(1, floor(--cpus-per-task/2))

  Environment variable equivalents: SCOUT_HOUGH_RUNS, REFINE_TOP_K, ALPHA_PARALLEL_WORKERS, NO_TWO_PHASE=1
...
SCOUT_HOUGH_RUNS, REFINE_TOP_K, ALPHA_PARALLEL_WORKERS, NO_TWO_PHASE
USAGE
}
...
# OMP/BLAS thread counts must be 1: forked alpha workers are already parallel;
# nested threading would oversubscribe and degrade performance.
export OMP_NUM_THREADS="${OMP_NUM_THREADS:-1}"
export OPENBLAS_NUM_THREADS="${OPENBLAS_NUM_THREADS:-1}"
export MKL_NUM_THREADS="${MKL_NUM_THREADS:-1}"
export NUMEXPR_NUM_THREADS="${NUMEXPR_NUM_THREADS:-1}"

# Resolve the real tuner_simple_alpha_sweep_pre_iou_levenshtein_parallel directory,
# even when Slurm runs a copied script from /var/spool/slurmd.
SCRIPT_SOURCE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Allow advanced users or Slurm wrappers to point directly at the _parallel directory.
...
# Use the current working directory when the Slurm job was submitted from tuner_simple_parallel itself.
...
elif [[ -f "/scratch/project_2017385/dorian/Churro_copy/tuner_simple_alpha_sweep_pre_iou_levenshtein_parallel/run_tuner_simple.py" ]]; then
  SCRIPT_DIR="/scratch/project_2017385/dorian/Churro_copy/tuner_simple_alpha_sweep_pre_iou_levenshtein_parallel"
else
  echo "ERROR: Could not locate run_tuner_simple.py. Set TUNER_SIMPLE_DIR=/path/to/tuner_simple_alpha_sweep_pre_iou_levenshtein_parallel." >&2
  exit 2
fi
...
build_cython_accelerators() {
  ...
  echo "[cython] building tuner_simple_parallel accelerators in ${SCRIPT_DIR}"
  if (cd "${SCRIPT_DIR}" && python3 cython_accel/build.py build_ext --inplace >/dev/null); then
    echo "[cython] tuner_simple_parallel accelerators are ready"
  else
...
# Two-phase parallel options
append_env_default "--scout-hough-runs" "SCOUT_HOUGH_RUNS"
append_env_default "--refine-top-k" "REFINE_TOP_K"
append_env_default "--alpha-parallel-workers" "ALPHA_PARALLEL_WORKERS"
# NO_TWO_PHASE=1 disables two-phase; any other value (or unset) keeps it enabled.
if [[ "${NO_TWO_PHASE:-0}" == "1" ]] && ! has_option "--no-two-phase" && ! has_option "--two-phase"; then
  python_arguments+=("--no-two-phase")
fi
...
# Launch the parallel tuner Python entry point with the assembled argument list.
