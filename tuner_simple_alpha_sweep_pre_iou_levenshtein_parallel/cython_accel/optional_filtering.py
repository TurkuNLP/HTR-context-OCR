...
try:
    from .filter_core import (
        ...
        unique_reference_rows_from_path_slice as _cython_unique_reference_rows_from_path_slice,
    )
except Exception:
    ...
    _cython_unique_reference_rows_from_path_slice = None
...
def unique_reference_rows_from_path_slice(
    ...
    """Return deduplicated reference rows for a column slice of a sampled path.

    Uses a C uint8 seen-array when the Cython extension is available; falls back
    to a Python set otherwise.  ``row_start`` and ``row_end`` are slice indices
    into ``sampled_reference_rows`` (not column numbers).
    """
    if _cython_unique_reference_rows_from_path_slice is not None:
        try:
            return list(
                _cython_unique_reference_rows_from_path_slice(
                    sampled_reference_rows,
                    int(row_start),
                    int(row_end),
    ...
    rows_slice = sampled_reference_rows[int(row_start):int(row_end)]
    seen: set[int] = set()
    unique_rows: list[int] = []
    ref_count = int(reference_window_count)
    for r in rows_slice:
        ri = int(r)
        if 0 <= ri < ref_count and ri not in seen:
            seen.add(ri)
