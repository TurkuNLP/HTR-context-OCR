...
class PipelineConfig:
    ...
    # Two-phase scout+refine: cheaply rank every alpha with scout_hough_runs, then re-run only
    # the top refine_top_k alphas at full hough_num_runs.  Disabled automatically when
    # scout_hough_runs >= hough_num_runs, refine_top_k >= num_active_alphas, or num_active <= 1.
    two_phase_enabled: bool = True
    # Number of Hough runs per alpha in the cheap scout pass.
    scout_hough_runs: int = 3
    # How many top-ranked scout alphas are re-evaluated at full hough_num_runs for final selection.
    refine_top_k: int = 5
    # Worker processes for parallel alpha evaluation. 0 = auto = max(1, floor(cpus/2)).
    alpha_parallel_workers: int = 0
    ...
    def validate(self) -> "PipelineConfig":
        ...
        if int(self.scout_hough_runs) < 1:
            raise ValueError("--scout-hough-runs must be at least 1")
        if int(self.refine_top_k) < 1:
            raise ValueError("--refine-top-k must be at least 1")
        if int(self.alpha_parallel_workers) < 0:
            raise ValueError("--alpha-parallel-workers must be zero or positive")
