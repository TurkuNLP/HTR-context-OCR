...
from tuner_simple_alpha_sweep_pre_iou_levenshtein_parallel.scoring.coverage_count_metrics import (
...
def test_missing_ref_to_ref_uses_full_diagonal_baseline():
    result = compute_coverage_count_metrics(
        ref_to_pred_scoring_payload=payload([short_top_left_line()]),
        ref_to_ref_scoring_payload=None,
    )

    assert result.correct_ref_coverage == 1.0 / 3.0
    assert result.missing_ref_coverage == 2.0 / 3.0
    assert result.repetition_on_reference == 0.0
    assert result.hallucination == 2.0 / 3.0
    assert result.diagnostics["coverage_y_diff_min"] == -1
    assert result.diagnostics["coverage_y_diff_max"] == 0


def test_partial_ref_to_ref_is_lifted_to_full_diagonal_baseline():
    result = compute_coverage_count_metrics(
        ref_to_pred_scoring_payload=payload([diagonal_line()]),
        ref_to_ref_scoring_payload=payload([short_top_left_line()]),
    )
    ...
    assert result.repetition_on_reference == 0.0
    assert result.hallucination == 0.0
    assert result.diagnostics["coverage_y_diff_min"] == 0
    assert result.diagnostics["coverage_y_diff_max"] == 0
